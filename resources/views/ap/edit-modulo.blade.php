<form class="form-group" enctype="multipart/form-data" id="edit-ap-modulo" method="POST">
...
<script>
    $("#edit-ap-modulo").on("submit", function(e){
        e.preventDefault();

        var cotizacion_id = $("#cotizacion_id").val();
        var formData = new FormData(document.getElementById("edit-ap-modulo"));
        formData.append("cotizacion_id", cotizacion_id);
        $.ajax({
            type: "POST",
            dataType: "html",
            url: "{{ url('/ap/update_modulo') }}",
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function (data) {
                $("#ap-modulo").empty();
                $("#ap-modulo").html(data);
                alert("Ap de módulo editado exitosamente");
            }
        });
    });
</script>
