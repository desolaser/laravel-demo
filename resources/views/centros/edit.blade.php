...
@section('content')
	<div class="col-md-10">
		<div class="box box-info">
			...
			<form action="{{ url("centros/{$data->id}") }}" method="POST" class="form-horizontal">
				@csrf
                @method('PUT')
				<div class="box-body">
						...
						<div class="col-sm-10">
							<select name="empresa_id" id="empresa_id" class="form-control">
						        ...
						        @foreach($empresas as $empresa)
						        	<option value="{{ $empresa->id }}" {{ old('empresa_id', $data->empresa_id == $empresa->id) ? 'selected' : '' }}>
						                {{ $empresa->nombre }}
						            </option>
						        @endforeach
						    </select>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="number" class="form-control" id="zona" name="zona" value="{{ old('zona', $data->zona) }}" required>
						</div>
					</div>
